...
import {post,get,put,deletefn} from '../../network/request'
export default {
  name: 'Params',
  ...
  methods: {
    ...
    async getParamsData() {
      ...
      const res = await get(`categories/${this.cateId}/attributes`, {
        sel: this.activeName 
      })
      console.log(res,111);
      if (res.meta.status !== 200) {
    ...
    addParams() {
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) return
        const res = await post(`categories/${this.cateId}/attributes`, {
    ...
    async showEditDialog(id) {
      const res = await get(`categories/${this.cateId}/attributes/${id}`, {
    ...
    editParams() {
      this.$refs.editFormRef.validate(async valid => {
        if (!valid) return
        const res = await put(`categories/${this.cateId}/attributes/${this.editForm.attr_id}`, {
    ...
    async removeParams(id) {
      ...
      const res = await deletefn(`categories/${this.cateId}/attributes/${id}`)
      console.log(res)
      if (res.meta.status !== 200) {
    ...
    async saveAttrVals(row) {
      const res = await put(`categories/${this.cateId}/attributes/${row.attr_id}`, {
        attr_sel: this.activeName,
